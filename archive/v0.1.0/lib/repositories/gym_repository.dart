...
class GymRepository {
  ...
  ///
  /// Returns the same user-facing status string produced by the helper
  /// (DatabaseHelper.importSuccessMessage on success, or an error message).
  ///
  /// Refresh model: GymRepository is not a ChangeNotifier and holds no in-memory
  /// caches — every query hits the live DB. Because MainScreen swaps tab widgets
  /// on navigation (disposing and rebuilding each tab's State), the Workout,
  /// Exercises, and History tabs will reload from the freshly imported DB the
  /// next time the user navigates to them. The calling screen (ProfileScreen)
  /// must refresh its own mounted state after this returns.
  Future<String> importDatabase(String sourcePath) async {
    return await _databaseHelper.importDatabase(sourcePath);
  }
